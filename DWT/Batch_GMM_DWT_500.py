...
for fname in os.listdir(dirname):
    ...
    red_coeffs_aprox = np.expand_dims(red_coeffs / red_coeffs.max() * 255, axis=-1)
    gre_coeffs_aprox = np.expand_dims(gre_coeffs / gre_coeffs.max() * 255, axis=-1)
    blu_coeffs_aprox = np.expand_dims(blu_coeffs / blu_coeffs.max() * 255, axis=-1)
    nir_coeffs_aprox = np.expand_dims(nir_coeffs / nir_coeffs.max() * 255, axis=-1)
    inverse = np.concatenate((nir_coeffs_aprox, red_coeffs_aprox, gre_coeffs_aprox, blu_coeffs_aprox), axis=-1)
    ...
    np.save(r"E:\Date\Teste\7_MDPI\Data\DWT\500/" + os.path.splitext(fname)[0] + ".npy", inverse)
    ...
    cv2.imwrite(r"E:\Date\Teste\7_MDPI\Output\DWT\500_Resized/" + os.path.splitext(fname)[0] + ".TIF", segmented_image)

    t5 = time.time()
    prediction_time_seconds = t5 - t4

    labels = labels.reshape((inverse.shape[0], inverse.shape[1]))
    np.save(r"E:\Date\Teste\7_MDPI\Labels\DWT\500/" + os.path.splitext(fname)[0] + ".npy", labels)
    labels = cv2.resize(labels, (image.shape[0], image.shape[1]), interpolation=cv2.INTER_NEAREST)
    np.save(r"E:\Date\Teste\7_MDPI\Labels\DWT\500_Resized/" + os.path.splitext(fname)[0] + ".npy", labels)
