...
for fname in os.listdir(dirname):
    ...
    nir_coeffs_3, _ = pywt.wavedec2(nir_coeffs_2, wavelet, level=1)
    red_coeffs_3, _ = pywt.wavedec2(red_coeffs_2, wavelet, level=1)
    gre_coeffs_3, _ = pywt.wavedec2(gre_coeffs_2, wavelet, level=1)
    blu_coeffs_3, _ = pywt.wavedec2(blu_coeffs_2, wavelet, level=1)
    red_coeffs_3_aprox = np.expand_dims(red_coeffs_3 / red_coeffs_3.max() * 255, axis=-1)
    gre_coeffs_3_aprox = np.expand_dims(gre_coeffs_3 / gre_coeffs_3.max() * 255, axis=-1)
    blu_coeffs_3_aprox = np.expand_dims(blu_coeffs_3 / blu_coeffs_3.max() * 255, axis=-1)
    nir_coeffs_3_aprox = np.expand_dims(nir_coeffs_3 / nir_coeffs_3.max() * 255, axis=-1)
    inverse = np.concatenate((nir_coeffs_3_aprox, red_coeffs_3_aprox, gre_coeffs_3_aprox, blu_coeffs_3_aprox), axis=-1)
    ...
    np.save(r"E:\Date\Teste\7_MDPI\Data\DWT\125/" + os.path.splitext(fname)[0] + ".npy", inverse)
    ...
    cv2.imwrite(r"E:\Date\Teste\7_MDPI\Output\DWT\125_Resized/" + os.path.splitext(fname)[0] + ".TIF", segmented_image)

    t5 = time.time()
    prediction_time_seconds = t5 - t4

    labels = labels.reshape((inverse.shape[0], inverse.shape[1]))
    np.save(r"E:\Date\Teste\7_MDPI\Labels\DWT\125/" + os.path.splitext(fname)[0] + ".npy", labels)
    labels = cv2.resize(labels, (image.shape[0], image.shape[1]), interpolation=cv2.INTER_NEAREST)
    np.save(r"E:\Date\Teste\7_MDPI\Labels\DWT\125_Resized/" + os.path.splitext(fname)[0] + ".npy", labels)
